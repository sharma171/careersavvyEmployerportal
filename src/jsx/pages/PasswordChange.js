        ...
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content position-relative">
            ...
            <div className="modal-header border-0 flex-column">
              ...
              <h5 className="h3" style={{textAlign:"center"}}>Password Updated!</h5>
              <p style={{textAlign:"center"}}>Your new password has been set successfully. You can now sign in with your updated credentials.</p>
...
function ForgotPassword() {
  ...
  const [isTemporaryPassword, setIsTemporaryPassword] = useState(false); // New state to track if this is temp password update
  ...
  useEffect(() => {
    ...
    const tempPassword = queryParams.get("temp"); // Check for temp password parameter
    const emailFromUrl = queryParams.get("email"); // Get email from URL for temp password
    
    if (tempPassword === "true" && emailFromUrl) {
      // This is a temporary password update
      setIsTemporaryPassword(true);
      setEmail(emailFromUrl);
    } else if (tokenFromUrl) {
      // This is a token-based password reset
  ...
  const handleSubmit = async () => {
    ...
    if (isTemporaryPassword) {
      // Use the temporary password update API
      payload = {
      ...
      // Use the existing token-based password reset API
      payload = {
        ...
        <div style={{ zIndex: 2, position: "relative" }} className="d-flex flex-column justify-content-center h-100">
          ...
          <div className="mt-1">
            <h1 className={`text-white fw-bold ${styles.headingText}`}>
              {isTemporaryPassword ? "Update Your Password" : "Reset Your Password"}
            </h1>
          </div>
          <div className={`mt-3 ${styles["text-lg"]} ${styles.mainText}`}>
            {isTemporaryPassword ? (
              <>
                Welcome to CareerSavvy.ai!
                <br />
    ...
    return (
      ...
      >
        <div className={styles.innerRight}>
          ...
          {/* Show email for temporary password updates */}
          {/* {isTemporaryPassword && (
            <div className="mb-4">
              <label>Email</label>
              <input
          ...
          <button className={styles.actionBtn} style={{ background: "#9333ea" }} onClick={handleSubmit}>
            {isTemporaryPassword ? "Update Password" : "Update Password"}
            <FaArrowRight />
          </button>
